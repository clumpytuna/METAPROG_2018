...
#define TYPELIST_3( T1, T2, T3 ) CTypeList<T1, TYPELIST_2( T2, T3 ) >
...
template <class TList, unsigned int i, typename DefaultType = NullType>
class GetTypeAt {
public:
...
// Класс - "функция". Предназначен для вырезания указанной части списка.
// typedef typename Slice<TypeList<H, T>, index>::Result
// где Result - новый typelist.
template <class CTypeList, unsigned int i, typename DefaultType=NullType>
struct Slice {
	typedef DefaultType Result;
};

template <class Head, class Tail, typename DefaultType>
struct Slice<CTypeList<Head, Tail>, 0, DefaultType> {
	typedef CTypeList<Head, Tail> Result;
};


template <class Head, class Tail, unsigned int i, typename DefaultType>
struct Slice<CTypeList<Head, Tail>, i, DefaultType> {
	typedef typename Slice<Tail, i - 1, DefaultType>::Result Result;
};


/////////////////////////////////////////////////////////////////////////////////////

// Класс - "функция". Предназначен для получения размера типов в тайплисте в байтах.
template <class CTypeList>
struct SizeOf;

template <class Head, class Tail>
struct SizeOf<CTypeList<Head, Tail> > {
	static const size_t result = sizeof(Head) + SizeOf<Tail>::result;
};
